      ...
      <el-form :model="addForm"
        ...
        <el-tabs v-model="activeIndex"
          ...
          <el-tab-pane label="商品图片"
                       name="3">
            <!-- action 表示图片要上传到的后台API地址 on-preview 表示点击预览图片-->
...
export default {
  ...
  methods: {
    ...
    // 处理移除图片的操作，file是将要给移除的图片，此字符由element-ui提供
    handleRemove (file) {
      // console.log(file)
      // 1. 获取将要删除的图片的临时路径
      const filePath = file.response.data.tmp_path
      // 2. 从 pics 数组中，找到这个图片对应的索引值，x代表数组里的每一项
